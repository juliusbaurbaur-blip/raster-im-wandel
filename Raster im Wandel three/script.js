...
(function () {
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (key === '1') {
      ...
      // Schedule a gentle, sequential revert for cells modified by LABIL
      const stableIdx = [];
      for (let i = 0; i < cells.length; i++) {
        if (cells[i].modifiedBy === 'stable') stableIdx.push(i);
      }
      // randomize order so reverts don't happen left->right/top->down
      stableIdx.sort(() => Math.random() - 0.5);
      // shorten overall times but keep randomness and some long-tail
      const baseDelayL = 0.12; // seconds before first revert
      const staggerL = 0.06; // nominal spacing between subsequent reverts
      const jitterL = 0.5; // per-cell jitter
      for (let k = 0; k < stableIdx.length; k++) {
        const idx = stableIdx[k];
        const linear = baseDelayL + k * staggerL;
        const longTail = Math.pow(Math.random(), 2) * 0.9; // shorter long-tail
        const spread = (Math.random() - 0.5) * jitterL;
        const delay = Math.max(0.02, linear + spread + longTail);
        const cell = cells[idx];
        // start slow reverting animation
        cell.targetAngle = 0;
        cell.reverting = true;
        cell.revertSpeedMult = 0.25 + Math.random() * 0.35; // vary slow animation speed a bit
        cell.flutterActive = false;
        // set the timer when the final revert (to white/square) should happen
        cell.revertTimer = delay + 0.6 + Math.random() * 0.8; // allow variable time for slow animation
      }
    }
    if (key === '2') {
      MODE = 'stable';
      ATTENUATION_DEGS = buildAttenuationDegs();
      HORIZ_RADIUS = ATTENUATION_DEGS.length - 1;
      console.log('Mode: LABIL');
      // If we switch back to LABIL, cancel any ongoing/staged reverts for cells
      // that were originally modified by LABIL so they remain blue and interactive.
      for (let i = 0; i < cells.length; i++) {
        const cell = cells[i];
        if (cell.modifiedBy === 'stable') {
          ...
          // restore accent color if a partial revert had set it to white
          if (cell.color === '#ffffff') cell.color = '#3a5eff';
        }
      }
      // Schedule a gentle, sequential revert for cells modified by STABLE
      const labilIdx = [];
      for (let i = 0; i < cells.length; i++) {
      ...
      // shorten overall times while keeping randomness
      const baseDelay = 0.06; // seconds before first revert
      const stagger = 0.04; // nominal spacing between subsequent reverts
      const jitter = 0.35; // per-cell jitter
      for (let k = 0; k < labilIdx.length; k++) {
        const idx = labilIdx[k];
        const linear = baseDelay + k * stagger;
        const longTail = Math.pow(Math.random(), 2) * 0.6; // shorter long-tail
